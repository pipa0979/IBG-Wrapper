...
int main(int argc, char* argv[])
{
	...
	for(int i=1;i<argc;i++){
		...
		if( strncmp(argv[i], "-min_m", strlen("-min_m")) == 0 && i < argc-1)				MIN_MATCH_LEN = atof(argv[++i]);
                ...
                else if( strcmp(argv[i], "-mapfile") == 0 && i < argc-1) 
                           MAPFILE = argv[++i];
                else if( strncmp(argv[i], "-outfile", 
                             strlen("-outfile")) == 0 && i < argc-1)
                                  OUTFILE = argv[++i];
		else if( strncmp(argv[i], "-bits", strlen("-bits")) == 0 && i < argc-1)				MARKER_SET_SIZE = atoi(argv[++i]);

		else if( strncmp(argv[i], "-err_w", strlen("-err_w")) == 0 && i < argc-1)                  
       ERR_W = atoi(argv[++i]);
